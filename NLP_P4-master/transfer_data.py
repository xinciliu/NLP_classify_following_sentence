...
def read_csv_file(file):
    ...
    with open(file,'r',encoding='ISO-8859-1') as csvfile:
        reader = csv.reader(csvfile)
        column7 = [row[7]for row in reader][1:]      
    return (column0,column1,column2,column3,column4,column5,column6,column7)  
...
def write_new_file(file):
    ...
    return (a[0],question_line,a[1],a[2],a[3],a[4],a[5],a[6],a[7])    
...
def write_csv_file(file):
    ...
    df2 = pd.DataFrame({'id': pd.Series(a[0]),
        ...
        'ending2':pd.Series(a[7]),'label':pd.Series(a[8])})
    df2.to_csv('dev.csv',index=False,columns=['id','question','sentence1',
